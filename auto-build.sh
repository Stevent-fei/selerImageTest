...
workdir="$(mktemp -d auto-build-XXXXX)" && sudo cp -r context1 "${workdir}" && cd "${workdir}/context1" && sudo cp -rf "${cri}"/* .
...
sudo ./sealer build -t "docker.io/18791106690/kubernetes:${k8s_version}" -f Kubefile --platform "${platform}" .
if [[ "$push" == "true" ]]; then
