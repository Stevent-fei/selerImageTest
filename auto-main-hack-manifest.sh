...
if [[ -z "$buildName" ]]; then
  buildName="docker.io/18791106690/kubernetes:${k8s_version}-hack"
...
workdir="$(mktemp -d auto-build-XXXXX)" && sudo cp -r context-manifests "${workdir}" && cd "${workdir}/context-manifests" && sudo cp -rf "${cri}"/* .
...
sudo sealer build -t "docker.io/18791106690/kubernetes:${k8s_version}-hack" -f Kubefile --platform "${platform}"
if [[ "$push" == "true" ]]; then
