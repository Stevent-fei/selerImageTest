...
if [[ -z "$buildName" ]]; then
  buildName="docker.io/18791106690/kubernetes-arm64:${k8s_version}-arm-multi"
...
workdir="$(mktemp -d auto-build-XXXXX)" && sudo cp -r context-main-arm "${workdir}" && cd "${workdir}/context-main-arm" && sudo cp -rf "${cri}"/* .
...
sudo sealer build -f Kubefile -t "docker.io/18791106690/kubernetes-arm64:${k8s_version}-arm-multi" --platform linux/amd64,linux/arm64
if [[ "$push" == "true" ]]; then
  if [[ -n "$username" ]] && [[ -n "$password" ]]; then
    sudo sealer login "$(echo "docker.io" | cut -d "/" -f1)" -u "${username}" -p "${password}"
  fi
  sudo sealer alpha manifest push "docker.io/18791106690/kubernetes-arm64:${k8s_version}-arm-multi" --all
fi
